...
import { ChevronRight, ArrowRight, Phone, MapPin, Award, ExternalLink } from 'lucide-react';
...
export default async function SupplierPage({
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '45vh' }}>
        ...
        >
          <div className="py-20 lg:py-24">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  {supplier.website && (
                    <a
                      ...
                      className="flex items-center gap-1.5 text-sm text-white/50 transition-colors hover:text-gold-light"
                    >
                      <ExternalLink size={13} />
                      <span>{l === 'es' ? 'Sitio oficial' : 'Official site'}</span>
                    </a>
                  )}
                </div>
              </div>
            ...
            {/* Cert badges in hero – use heroBadges if defined, else all certs */}
            {(supplier.heroBadges ?? supplier.certifications.map((c) => c.name)).length > 0 && (
              <CertBadges
                badges={supplier.heroBadges ?? supplier.certifications.map((c) => c.name)}
                variant="light"
              ...
              <div>
                ...
                {supplier.website && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-navy transition-colors hover:text-gold"
                  >
                    <ExternalLink size={14} />
                    {l === 'es' ? `Visitar sitio oficial de ${supplier.name[l]}` : `Visit ${supplier.name[l]} official site`}
                  </a>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    {supplier.productCategories.map((cat) => {
                      const industryLabels: Record<string, { es: string; en: string }> = {
                        'oil-gas': { es: 'Oil & Gas', en: 'Oil & Gas' },
                        energetico: { es: 'Energético', en: 'Power Generation' },
                        privado: { es: 'Industria General', en: 'General Industry' },
                      };
                      return (
                        <div
                          key={cat.title[l]}
                        ...
                        >
                          ...
                          {cat.industrias.length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-1.5">
                              {cat.industrias.map((ind) => (
                                <Link
                                  key={ind}
                                  href={`${prefix}/industrias/${ind}`}
                                  className="rounded-md bg-navy-deep/[0.06] px-2.5 py-1 text-[11px] font-medium text-navy transition-colors hover:bg-gold/10 hover:text-gold"
                                >
                                  {industryLabels[ind]?.[l] ?? ind}
                                </Link>
                              ))}
                            </div>
                          )}
                ...
                <div className="rounded-xl border border-gray-100 bg-white p-7 shadow-md">
                  ...
                  <div className="space-y-1">
                    {otherSuppliers.map((s) => (
                      <Link
                        key={s.slug}
                        href={`${prefix}/fabricantes/${s.slug}`}
      ...
      {/* PAS Interconnect CTA */}
      {supplier.pas.ctaInterconnect && (
        <section className="bg-gray-50 pb-16">
          <div className="mx-auto max-w-6xl px-6 text-center">
            <Link
              href={`${prefix}${supplier.pas.ctaInterconnectLink}`}
              className="group inline-flex items-center gap-2 rounded-full border border-gold/30 bg-white px-8 py-3.5 text-sm font-semibold text-gold shadow-sm transition-all duration-150 hover:border-gold hover:bg-gold hover:text-white hover:shadow-md"
            >
              <ExternalLink size={15} className="transition-transform group-hover:scale-110" />
              {supplier.pas.ctaInterconnect[l]}
            </Link>
          </div>
        </section>
      )}
